...
- [Debugging] Fix song desynchronization when switching between Vocabulary and Lyrics tabs: In `App.tsx`, eliminated tab-hijacking side effect (`setActiveTab('가사')` inside useEffect) that disrupted user studying in the Vocabulary tab. Updated `onMusicUpdate` to automatically keep `viewingSong` updated to the newly playing track in the background when not explicitly viewing historical archives. Added `isViewingHistory` tracking and immediate synchronization when clicking the '가사' navbar tab, ensuring returning to lyrics immediately displays the currently playing Apple Music track without remaining stuck on the previous song or needing repeated tab navigation.
